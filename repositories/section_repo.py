...
async def complete_section_repo(student_id: int, sectiond_id: int, insert_data_func = insert_query):
    ...
    """
    return await insert_data_func(query, (student_id, sectiond_id))
...
async def get_completed_sections_repo(student_id: int, course_id: int, get_data_func=read_query):
    ...
    WHERE scs.students_id = $1 AND cs.course_id = $2 AND scs.is_completed = TRUE
    """
